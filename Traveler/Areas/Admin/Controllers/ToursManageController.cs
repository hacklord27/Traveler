...
using Traveler.Design_pattern.Stragey_pattern;
    ...
    public class ToursManageController : Controller
    {
        ...
        public ActionResult Create(Tour tour, HttpPostedFileBase Image1, HttpPostedFileBase Image2, HttpPostedFileBase Image3)
        {
            if (ModelState.IsValid)
            {
                ...
                //    return View(tour);
                //}
                IImageStorageStrategy imageStorageStrategy = new DiskImageStorageStrategy(); // or CloudImageStorageStrategy()

                // Save images using the selected strategy
                imageStorageStrategy.SaveImage(Image1, Server.MapPath("~/Content/img"));
                imageStorageStrategy.SaveImage(Image2, Server.MapPath("~/Content/img"));
                imageStorageStrategy.SaveImage(Image3, Server.MapPath("~/Content/img"));
